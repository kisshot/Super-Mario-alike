...
class Mario(Sprite):
    ...
    def __init__(self, ai_settings, screen):
        ...
        # 将Mario放在屏幕底部中央
        self.rect.centerx = 1000 #self.screen_rect.centerx - 160
    ...
    def update(self):
        ...
        # if self.state == st.WALK_LEFT and self.rect.left > 0 :
        if self.moving_left and self.rect.left > 0:      # self.screen_rect.left
            self.image = self.small_normal_frames[1][0]
            self.center -= self.ai_settings.mario_speed_factor

        # if self.state == st.JUMP and self.rect.bottom > self.screen_rect.bottom - 88:
        if self.moving_up:
        #     if self.state == st.WALK_RIGHT:
        ...
        # if self.state == st.JUMP and self.rect.bottom < self.screen_rect.bottom - 24:
        if self.moving_down:
            self.falling()
