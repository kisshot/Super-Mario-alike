...
def handle_state(stats, marios, enemies):
    # spritecollideany
    # spritecollide
    if pygame.sprite.groupcollide(marios, enemies, False, True):
        # enemies.empty()  #'''这句话很关键'''
        print('THEY ARE TOGETHER+++++++++++++++++++++++++++++++++++++')
        # !!! mario_hit(stats, marios, enemies)

def mario_hit(stats, marios, enemies):
    """响应被敌人杀死的马里奥"""
    # !!! stats.game_active = False
    if stats.mario_left > 0:
        #将马里奥数量减去1
        stats.mario_left -= 1
...
def update_menu(screen, mario):
    ...
    screen.blit(mario.image, (50, 180))
    # enemy_shadow.blitme(centerx)
    # 让最近绘制的屏幕可见
    pygame.display.update()
